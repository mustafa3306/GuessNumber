...
myBtn.addEventListener("click", () => {
    if(myInp.value > lowest && myInp.value < highest){
        ...
        if(rasgeleSayi == myInp.value){
            myMsg.innerText = `${myInp.value} is right. Lucky. You won !`
        }
        else if(rasgeleSayi > myInp.value){
            myMsg.innerText = `${myInp.value} is wrong. Enter a number between ${myInp.value} and ${highest}`;
            lowest = myInp.value
               
        }
        else if(rasgeleSayi < myInp.value){
            myMsg.innerText = `${myInp.value} is wrong. Enter a number between ${lowest} and ${myInp.value}`
            highest = myInp.value
        }
    }else{
